...
const SportPage = () => {
  ...
  const totalItems = LengthOfCategories?.lengthOfSports || 0; // Ensure fallback for undefined LengthOfCategories
  const totalPages = Math.ceil(totalItems / itemsPerPage);

  // Fetch sports articles for pagination
  const handlePaginationSports = async (startIndex, endIndex) => {
    setLoading(true);
    try {
      ...
      if (res.status === 200) {
        if (endIndex === 23) {
          // localStorage.setItem("sports", JSON.stringify(res.data.list));
          setSports(res.data.list);
          return;
        }
        // const updatedCache = [...sports, ...res.data.list];
        // localStorage.setItem("sports", JSON.stringify(updatedCache));
        setSports((prev) => {
  ...
  // Initial load
  useEffect(() => {
    if (endIndexForSports === 23 && sports.length === 0) {
      setLoading(true);
      handlePaginationSports(0, 23);
    }
  }, []);

  // Handle page changes
  const handlePageChange = (pageNum) => {
    setCurrentPagePageSport(pageNum);
    ...
    if (fetchLimit > endIndexForSports) {
      handlePaginationSports(endIndexForSports, fetchLimit);
      setEndIndexForSports(fetchLimit);
      // Cache the new endIndex in localStorage
      // localStorage.setItem("endIndexForSports", fetchLimit);
    }
  };

  // Get current page items
  const getCurrentItems = () => {
    const startIndex = currentPageSport * itemsPerPage;
    return sports?.slice(startIndex, startIndex + itemsPerPage);
  };
