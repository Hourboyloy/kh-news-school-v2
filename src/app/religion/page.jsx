...
const ReligionPage = () => {
  ...
  const totalItems = LengthOfCategories?.lengthOfLifes || 0; // Ensure fallback for undefined LengthOfCategories
  const totalPages = Math.ceil(totalItems / itemsPerPage);

  // Fetch sports articles for pagination
  const handlePaginationLifes = async (startIndex, endIndex) => {
    setLoading(true);
    try {
      ...
      if (res.status === 200) {
        if (endIndex === 23) {
          // localStorage.setItem("lifes", JSON.stringify(res.data.list));
          setLifes(res.data.list);
          return;
        }
        // const updatedCache = [...lifes, ...res.data.list];
        // localStorage.setItem("lifes", JSON.stringify(updatedCache));
  ...
  // Initial load
  useEffect(() => {
    if (endIndexForlifes === 23 && lifes.length === 0) {
      setLoading(true);
      handlePaginationLifes(0, 23);
    }
  }, []);

  // Handle page changes
  const handlePageChange = (pageNum) => {
    setCurrentPagePageReligion(pageNum);
    ...
    if (fetchLimit > endIndexForlifes) {
      handlePaginationLifes(endIndexForlifes, fetchLimit);
      setEndIndexForlifes(fetchLimit);

      // Cache the new endIndex in localStorage
      // localStorage.setItem("endIndexForlifes", fetchLimit);
    }
  };

  // Get current page items
  const getCurrentItems = () => {
    const startIndex = currentPageReligion * itemsPerPage;
    return lifes?.slice(startIndex, startIndex + itemsPerPage);
  };
  ...
  return (
    <div className="w-full pb-7">
      ...
      <Pagination
        ...
        currentPage={currentPageReligion}
